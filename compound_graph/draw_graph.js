    ...
    .then((dot_graph) => {
        ...
        $(window).on("mousemove", function(window_event){ 
            cy.nodes().on("mouseover", function(cy_event){
                ...
                if(childrenData.get(cy_event.target.data("id")).node.length > 0)document.getElementById("name-plate").textContent = childrenData.get(cy_event.target.data("id")).node
